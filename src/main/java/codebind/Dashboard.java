...
public class Dashboard extends Library {
    ...
    private final ElementsCollection TABLE_PROJECT_CSS = $$(".table.summary.summary-auto .row.project.flex-projects-row");
    ...
    public Dashboard findProjectAndClick(String projectName) {
        if (TABLE_PROJECT_CSS.first().isDisplayed()) {
            try {
                TABLE_PROJECT_CSS.filterBy(Condition.text(projectName))
                        .first()
                        .$(".summary-links.text-secondary a:nth-child(4)")
                        .click();
            } catch (Exception e) {
    ...
    public Dashboard findSuitsAndClick(String suitName) {
        if (TABLE_SUITNAMES_CSS.first().isDisplayed()) {
            try {
                TABLE_SUITS_CSS.filterBy(Condition.text(suitName))
                        .first()
                        .$("a")
                        .click();
            } catch (Exception e) {
    ...
    public Dashboard clickAndSelectImprot(String importType) {
        try {
            ...
            DRP_SUITS_CSS.filterBy(Condition.text(importType))
                    .first()
                    .$("a")
                    .click();
        } catch (Exception e) {
    ...
    public Dashboard importCsv(String pathCSV) {
        try {
            ...
            if (MSG_importCsvErrors_CSS.isDisplayed()) {
                Toolkit.getDefaultToolkit().beep();
                JOptionPane.showMessageDialog(null, "The Title field is a required field in TestRail and must be mapped to a CSV column.", "Error", -1);
                closeFail();
            }
